...
function logCsvZEF(file, msg)
   ...
   if file4 == nil then
      ...
      -- log format ts, dg01Pac, dg02Pac, pvPac, pacLimit, gridConnSt, pacLimitSet, gridConnSet
      file4:write("TS,CASE,ZE_FAC,ZE_PAC,PV_PAC,PAC_LIMIT,GRID_CONNECT,PAC_LIMIT_WRITE,GRID_CONNECT_WRITE".."\n")
   end
   file4:close()
...
function logCsvZEPF(file, msg)
   ...
   if file5 == nil then
      ...
      -- log format ts, dg01Pac, dg02Pac, pvPac, pacLimit, gridConnSt, pacLimitSet, gridConnSet
      file5:write("TS,CASE,ZE_QAC,ZE_PF,PV_QAC,QAC_LIMIT,GRID_CONNECT,QAC_LIMIT_WRITE,GRID_CONNECT_WRITE".."\n")
   end
   file5:close()
...
if (dev == lastDev) then
 if ampZE01FUHF == nil then
  ...
  ampZE01FUHF =
  function(dev)
    local pacMaxSetCmd = (WR.read(dev, "PAC_MAX_SET_CMD")/1.28571429)
    local qacMaxSetCmd = (WR.read(dev, "QAC_MAX_SET_CMD")/1.28571429)
    local blkPacMaxCmd = (WR.read(dev, "BLOCK_PAC_MAX_CMD")/1.28571429)
    local blkQacMaxCmd = (WR.read(dev, "BLOCK_QAC_MAX_CMD")/1.28571429)
    ...
    local fDroopCmd = WR.read(dev, "F_DROOP_CMD")
   ...
   local zePac = (WR.read(dev, "ZE_PAC") * (6.3/(21.150))) * 1000
   ...
   local zePMinLoad = (WR.read(dev, "PAC_PC_CMD") * (6.3/(21.150))) * 1000
   local zePCriticalLoad = zePMinLoad - ((pacLLSetCmd / 100) * blkPacMaxCmd)

   local zeMinLoad = (WR.read(dev, "QAC_PC_CMD") * (6.3/(21.150))) * 1000
...
--[[
   ...
   local zeQac = ((WR.read(dev, "ZE_QAC")) * (6.3/(21.150))) * 1000
...
--[[
   ...
   --local inv4_Com = WR.read("SN:B02_INV02", "COMMUNICATION_STATUS")


   local inv3_Pac = WR.read("SN:B02_INV01", "PAC")
   --local inv4_Pac = WR.read("SN:B02_INV02", "PAC")
   ...
   --WR.setProp("SN:B02_INV02", "TOTAL_INV_ONLINE", totalInverters)
   
   local totalQInverters = 0

   if (inv3_Com < 1)  then totalQInverters = totalQInverters + 1 end
   --if (inv4_Com < 1)  then totalQInverters = totalQInverters + 1 end


   WR.setProp("SN:B02_INV01", "TOTAL_INV_ONLINE_Q", totalQInverters)
   --WR.setProp("SN:B02_INV02", "TOTAL_INV_ONLINE_Q", totalQInverters)
    ...
    elseif ((zeIac >= zeIacSetCmd) and (maxPacLimit < pacMaxSetCmd)) then
     if (zeIac > zeIacMaxCmd) then
	  ...
	  logCsv(fileName,"1.4.0"..","..zePac..","..pvPac..","..oldPacLimit..","..gridConn..","..pacLimit..",".."")
	      ...
	      case4ZE = case4ZE + 1
	      logCsv(fileName,"1.4.1`"..","..zePac..","..pvPac..","..oldPacLimit..","..gridConn..","..pacLimit..",".."")
         end
       end
     else
        case5ZE = case5ZE + 1
        logCsv(fileName,"1.5.0"..","..zePac..","..pvPac..","..oldPacLimit..","..gridConn..","..",".."")
     end
   end
  ...
  if ((not(is_nan(zeQac))) and (PowSelCmd == 1) and (totalQInverters > 0) and (fOnOffCmd == 1) and (qacOnOffCmd == 1))then   -- checking Grid Reactive Power not nan
    ...
    if ((zeQac <= zeCriticalLoad) and (maxQacLimit > -100) and ((zePf > zePfSetCmd) or (zePf < -zePfSetCmd))) then
	 ...
	 if (case1ZEQ >= caseReset) then
       ...
       logCsvZE(fileNameZE,"1.1.0"..","..zeQac..","..pvQacW..","..oldQacLimitW..","..gridConn..","..qacLimitW..",".."")
      else
       case1ZEQ = case1ZEQ + 1
       logCsvZE(fileNameZE,"1.1.1"..","..zeQac..","..pvQacW..","..oldQacLimitW..","..gridConn..","..qacLimitW..",".."")
      end
     else
	   oldQacLimitW = tonumber(string.format("%.1f", (1 * (oldQacLimit / qacMaxSetCmd) * 100)))
       qacLimitW = tonumber(string.format("%.1f", (1 * (qacLimit / qacMaxSetCmd) * 100)))
      ...
      if (case1ZEQ >= caseReset) then
       ...
       logCsvZE(fileNameZE,"1.1.2"..","..zeQac..","..pvQacW..","..oldQacLimitW..","..gridConn..","..qacLimitW..",".."")
      else
       case1ZEQ = case1ZEQ + 1
       logCsvZE(fileNameZE,"1.1.3"..","..zeQac..","..pvQacW..","..oldQacLimitW..","..gridConn..","..qacLimitW..",".."")
      end
     end
    -- case 2 : ZE meter > Min Load  force up --  Reactive Power consumption from grid < -50VAR
     ...
     if (oldQacLimit >= qacLimit) then
	  ...
	  if (case2ZEQ >= caseReset) then
       ...
       logCsvZE(fileNameZE,"1.2.0"..","..zeQac..","..pvQacW..","..oldQacLimitW..","..gridConn..","..qacLimitW..",".."")
      else
       case2ZEQ = case2ZEQ + 1
       logCsvZE(fileNameZE,"1.2.1"..","..zeQac..","..pvQacW..","..oldQacLimitW..","..gridConn..","..qacLimitW..",".."")
      end
     else
      oldQacLimitW = tonumber(string.format("%.1f", (1 * (oldQacLimit / qacMaxSetCmd) * 100)))
      qacLimitW = tonumber(string.format("%.1f", (1 * (qacLimit / qacMaxSetCmd) * 100)))
	  ...
	  if (case2ZEQ >= caseReset) then
       ...
       logCsvZE(fileNameZE,"1.2.2"..","..zeQac..","..pvQacW..","..oldQacLimitW..","..gridConn..","..qacLimitW..",".."")
      else
       case2ZEQ = case2ZEQ + 1
       logCsvZE(fileNameZE,"1.2.3"..","..zeQac..","..pvQacW..","..oldQacLimitW..","..gridConn..","..qacLimitW..",".."")
      end
     end
   -- case 3 : ZE meter > minload & ZE meter < 10  tune up
    elseif ((zeQac <= (zeMinLoad - zeThreshold)) and (minQacLimit > -100) and ((zePf > zePfSetCmd) or (zePf < -zePfSetCmd))) then
	 ...
	 if (case3ZEQ >= caseReset) then
      ...
      logCsvZE(fileNameZE,"1.3.0"..","..zeQac..","..pvQacW..","..oldQacLimitW..","..gridConn..","..qacLimitW..",".."")
     else
      case3ZEQ = case3ZEQ + 1
      logCsvZE(fileNameZE,"1.3.1"..","..zeQac..","..pvQacW..","..oldQacLimitW..","..gridConn..","..qacLimitW..",".."")
     end
    --case 4 : ZE meter > 0 & ZE meter < minload  no change
    elseif (((zePf < 0) and (zePf >= -zePfSetCmd)) or ((zePf > 0) and (zePf <= zePfSetCmd)))  then --and (zePac <= zeThreshold)) then
          ...
          if (((zePf <= zePfMaxCmd) and (zePf >= -zePfMaxCmd)) and (maxQacLimit >= 0)) then
           ...
           if (qacLimit < -qacMaxSetCmd) then qacLimit = -qacMaxSetCmd end
			   ...
			   logCsvZE(fileNameZE,"1.4.0"..","..zeQac..","..pvQacW..","..oldQacLimitW..","..gridConn..","..qacLimitW..",".."")
			else 
			   case4ZEQ = case4ZEQ + 1
               logCsvZE(fileNameZE,"1.4.1"..","..zeQac..","..pvQacW..","..oldQacLimitW..","..gridConn..","..qacLimitW..",".."")
			end   
      elseif (((zePf <= zePfMaxCmd) and (zePf >= -zePfMaxCmd)) and (maxQacLimit < 0)) then
        ...
        qacLimit = tonumber(string.format("%.0f", qacLimit))
               ...
               logCsvZE(fileNameZE,"1.4.2"..","..zeQac..","..pvQacW..","..oldQacLimitW..","..gridConn..","..qacLimitW..",".."")
			else 
			   case4ZEQ = case4ZEQ + 1
               logCsvZE(fileNameZE,"1.4.3"..","..zeQac..","..pvQacW..","..oldQacLimitW..","..gridConn..","..qacLimitW..",".."")				
            end
		else	
          logCsvZE(fileNameZE,"1.5.1"..","..zeQac..","..pvQac..","..oldQacLimit..","..gridConn..","..qacLimit..",".."")
      end
	 else
               ...
               pvQacW = 1 * (pvQac)
               case5ZEQ = case5ZEQ + 1
               logCsvZE(fileNameZE,"1.5.0"..","..zeQac..","..zePf..","..pvQacW..","..oldQacLimitW..","..gridConn..","..",".."")
            end
         end
         ...
         -- case 1 : ZE meter < critical load (100VAR) ; Reactive Power supply to grid
          ...
          if (oldQacLimit <= qacLimit) then
           ...
           if (case1ZEPF >= caseReset) then
            ...
            logCsvZEPF(fileNameZEPF,"1.1.0"..","..zeQac..","..zePf..","..pvQacW..","..oldQacLimitW..","..gridConn..","..qacLimitW..",".."")
           else
            case1ZEPF = case1ZEPF + 1
            logCsvZEPF(fileNameZEPF,"1.1.1"..","..zeQac..","..zePf..","..pvQacW..","..oldQacLimitW..","..gridConn..","..qacLimitW..",".."")
           end
          else
           oldQacLimitW = tonumber(string.format("%.3f", (1 * (oldQacLimit / qacMaxSetCmd) * 100)))
           qacLimitW = tonumber(string.format("%.3f", (1 * (qacLimit / qacMaxSetCmd) * 100)))
           ...
           if (case1ZEPF >= caseReset) then
            ...
            logCsvZEPF(fileNameZEPF,"1.1.2"..","..zeQac..","..zePf..","..pvQacW..","..oldQacLimitW..","..gridConn..","..qacLimitW..",".."")
           else
            case1ZEPF = case1ZEPF + 1
            logCsvZEPF(fileNameZEPF,"1.1.3"..","..zeQac..","..zePf..","..pvQacW..","..oldQacLimitW..","..gridConn..","..qacLimitW..",".."")
           end
          end
          -- case 2 : ZE meter > Min Load  force up --  Reactive Power consumption from grid < -50VAR
          ...
          if (oldQacLimit >= qacLimit) then
           ...
           if (case2ZEPF >= caseReset) then
            ...
            logCsvZEPF(fileNameZEPF,"1.2.0"..","..zeQac..","..zePf..","..pvQacW..","..oldQacLimitW..","..gridConn..","..qacLimitW..",".."")
           else
            case2ZEPF = case2ZEPF + 1
            logCsvZEPF(fileNameZEPF,"1.2.1"..","..zeQac..","..zePf..","..pvQacW..","..oldQacLimitW..","..gridConn..","..qacLimitW..",".."")
           end
          else
           oldQacLimitW = tonumber(string.format("%.3f", (1 * (oldQacLimit / qacMaxSetCmd) * 100)))
           qacLimitW = tonumber(string.format("%.3f", (1 * (qacLimit / qacMaxSetCmd) * 100)))
           ...
           if (case2ZEPF >= caseReset) then
            ...
            logCsvZEPF(fileNameZEPF,"1.2.2"..","..zeQac..","..zePf..","..pvQacW..","..oldQacLimitW..","..gridConn..","..qacLimitW..",".."")
           else
            case2ZEPF = case2ZEPF + 1
            logCsvZEPF(fileNameZEPF,"1.2.3"..","..zeQac..","..zePf..","..pvQacW..","..oldQacLimitW..","..gridConn..","..qacLimitW..",".."")
           end
          end
         --case 4 : ZE meter > 0 & ZE meter < minload  no change
         else
            ...
            logCsvZEPF(fileNameZEPF,"1.5.0"..","..zeQac..","..zePf..","..pvQacW..","..oldQacLimitW..","..gridConn..","..",".."")
          end
      end
    ...
    if ((zeUac >= (zeUMinLoad + zeUThreshold)) and ((zePf > zePfSetCmd) or (zePf < -zePfSetCmd))) then
     ...
     elseif (oldQacLimit > qacMaxSet) then
      if (case1ZEU >= caseReset) then
       ...
       case1ZEU = case1ZEU + 1
       logCsvZEU(fileNameZEU,"1.1.4"..","..zeUac..","..zeQac..","..pvQac..","..oldQacLimit..","..gridConn..","..qacLimit..",".."")     	
     end
     ...
     elseif (minQacLimit < qacMaxSet) then
      if (case2ZEU >= caseReset) then
       ...
       logCsvZEU(fileNameZEU,"1.2.3"..","..zeUac..","..zeQac..","..pvQac..","..oldQacLimit..","..gridConn..","..qacLimit..",".."")
    	...
    	case2ZEU = case2ZEU + 1
       logCsvZEU(fileNameZEU,"1.2.4"..","..zeUac..","..zeQac..","..pvQac..","..oldQacLimit..","..gridConn..","..qacLimit..",".."")
     end
   -- case 3 : ZE meter > minload & ZE meter < 10  tune up
    elseif ((zeUac <= (zeUMinLoad - zeUThreshold)) and ((zePf > zePfSetCmd) or (zePf < -zePfSetCmd))) then
     ...
     elseif (minQacLimit < qacMaxSet) then
      if (case3ZEU >= caseReset) then
       ...
       logCsvZEU(fileNameZEU,"1.3.2"..","..zeUac..","..zeQac..","..pvQac..","..oldQacLimit..","..gridConn..","..qacLimit..",".."")
      else
       case3ZEU = case3ZEU + 1
       logCsvZEU(fileNameZEU,"1.3.3"..","..zeUac..","..zeQac..","..pvQac..","..oldQacLimit..","..gridConn..","..qacLimit..",".."")
      end
     else
       case3ZEU = case3ZEU + 1
       logCsvZEU(fileNameZEU,"1.3.4"..","..zeUac..","..zeQac..","..pvQac..","..oldQacLimit..","..gridConn..","..qacLimit..",".."")
     end
    ...
    elseif ((zeUac >= (zeUMinLoad - zeUThreshold)) and (zeUac <= (zeUMinLoad + zeUThreshold)) and ((zePf > zePfSetCmd) or (zePf < -zePfSetCmd))) then
               if (zeUac > zeUMinLoad) and (maxQacLimit > 0) then
                  ...
                  if (case6ZEU >= caseReset) then
                     ...
                     logCsvZEU(fileNameZEU,"1.6.0"..","..zeUac..","..zeQac..","..pvQac..","..oldQacLimit..","..gridConn..","..qacLimit..",".."")
                  else
                     case6ZEU = case6ZEU + 1
                     logCsvZEU(fileNameZEU,"1.6.1"..","..zeUac..","..zeQac..","..pvQac..","..oldQacLimit..","..gridConn..","..qacLimit..",".."")
                  end
               elseif (zeUac < zeUMinLoad) and (maxQacLimit < 0) then
                  ...
                  if (case6ZEU >= caseReset) then
                     ...
                     logCsvZEU(fileNameZEU,"1.6.2"..","..zeUac..","..zeQac..","..pvQac..","..oldQacLimit..","..gridConn..","..qacLimit..",".."")
                  else
                     case6ZEU = case6ZEU + 1
                     logCsvZEU(fileNameZEU,"1.6.3"..","..zeUac..","..zeQac..","..pvQac..","..oldQacLimit..","..gridConn..","..qacLimit..",".."")
                  end
               else
                  case7ZEU = case7ZEU + 1
                  logCsvZEU(fileNameZEU,"1.6.4"..","..zeUac..","..zeQac..","..pvQac..","..oldQacLimit..","..gridConn..","..qacLimit..",".."")
               end
    --case 4 : ZE meter > 0 & ZE meter < minload  no change
            elseif (((zePf < 0) and (zePf >= -zePfSetCmd)) or ((zePf > 0) and (zePf <= zePfSetCmd)))  then --and (zePac <= zeThreshold)) then
               ...
               if ((zePf <= zePfMaxCmd) and (zePf >= -zePfMaxCmd)) and (((maxQacLimit < 0) and (zeUac > (zeUMinLoad + zeUThreshold))) or ((maxQacLimit < 0) and (zeUac < (zeUMinLoad - zeUThreshold))) or ((maxQacLimit < 0) and (zeUac > (zeUMinLoad - zeUThreshold)) and (zeUac < (zeUMinLoad + zeUThreshold)))) then
                  ...
                  if (case4ZEU >= caseReset) then
                     ...
                     logCsvZEU(fileNameZEU,"1.4.0"..","..zeUac..","..zeQac..","..pvQacW..","..oldQacLimit..","..gridConn..","..qacLimit..",".."")
                  else
                     case4ZEU = case4ZEU + 1
                     logCsvZEU(fileNameZEU,"1.4.1"..","..zeUac..","..zeQac..","..pvQacW..","..oldQacLimit..","..gridConn..","..qacLimit..",".."")
                  end
               elseif ((zePf <= zePfMaxCmd) and (zePf >= -zePfMaxCmd)) and (((maxQacLimit > 0) and (zeUac > (zeUMinLoad + zeUThreshold))) or ((maxQacLimit > 0) and (zeUac < (zeUMinLoad - zeUThreshold))) or ((maxQacLimit > 0) and (zeUac > (zeUMinLoad - zeUThreshold)) and (zeUac < (zeUMinLoad + zeUThreshold)))) then
                  ...
                  if (case4ZEU >= caseReset) then
                     ...
                     logCsvZEU(fileNameZEU,"1.4.2"..","..zeUac..","..zeQac..","..pvQacW..","..oldQacLimit..","..gridConn..","..qacLimit..",".."")
                  else
                     case4ZEU = case4ZEU + 1
                     logCsvZEU(fileNameZEU,"1.4.3"..","..zeUac..","..zeQac..","..pvQacW..","..oldQacLimit..","..gridConn..","..qacLimit..",".."")
                  end
               else
                  case4ZEU = case4ZEU + 1
                  logCsvZEU(fileNameZEU,"1.4.4"..","..zeUac..","..zeQac..","..pvQacW..","..oldQacLimit..","..gridConn..","..qacLimit..",".."")
               end
            else
               --case5ZEU = case5ZEU + 1
               case5ZEU = case5ZEU + 1
               logCsvZEU(fileNameZEU,"1.5.0"..","..zeUac..","..zeQac..","..pvQacW..","..oldQacLimit..","..gridConn..","..qacLimit..",".."")
            end
   end
...
if ((not(is_nan(zeFac))) and (PowSelCmd == 1) and (totalInverters > 0) and (fOnOffCmd == 1) and (facOnOffCmd == 1)) then -- facOnOffCmd
            ...
            if ((zeFac > (zeFMinLoad + zeFThreshold)) and (zeFac <= (zeFMinLoad + zeFCriticalLoad)) and (maxPacLimit > 0) and (zeIac < zeIacSetCmd)) then
               ...
               pacLimit = ((pvPac + ((-1 * (zeFac - zeFMinLoad) * zePac) / (zeFMinLoad * fDroopCmd))) / totalInverters) / ((100 - facGRSetCmd) / 100)
               --if ((zePac / totalInverters) < pacLimit) then pacLimit = (maxPacLimit) end -- Gain control line added
               if (pacLimit <= 0) then pacLimit = 0 end
               pacLimit = tonumber(string.format("%.0f", pacLimit))
               oldPacLimit = tonumber(string.format("%.0f", (maxPacLimit)))
               if (oldPacLimit <= pacLimit) then
                  pacLimit = oldPacLimit - tonumber(string.format("%.0f", (facDecConstCmd * pacMaxSetCmd / 1000)))
                  if (case1ZEF >= caseReset) then
                     ...
                     logCsvZEF(fileNameZEF,"1.1.0"..","..zeFac..","..zePac..","..pvPac..","..oldPacLimit..","..gridConn..","..pacLimit..",".."")
                  else
                     case1ZEF = case1ZEF + 1
                     logCsvZEF(fileNameZEF,"1.1.1"..","..zeFac..","..zePac..","..pvPac..","..oldPacLimit..","..gridConn..","..pacLimit..",".."")
                  end
               else
                  if (case1ZEF >= caseReset) then
                     case1ZEF = 0
                     ...
                     logCsvZEF(fileNameZEF,"1.1.2"..","..zeFac..","..zePac..","..pvPac..","..oldPacLimit..","..gridConn..","..pacLimit..",".."")
                  else
                     case1ZEF = case1ZEF + 1
                     logCsvZEF(fileNameZEF,"1.1.3"..","..zeFac..","..zePac..","..pvPac..","..oldPacLimit..","..gridConn..","..pacLimit..",".."")
                  end
               end
            ...
            elseif ((zeFac < (zeFMinLoad - zeFThreshold)) and (zeFac >= (zeFMinLoad - zeFCriticalLoad)) and (minPacLimit <= pacMaxSetCmd) and (zeIac < zeIacSetCmd)) then
               ...
               oldPacLimit = tonumber(string.format("%.0f", (minPacLimit)))
               if (oldPacLimit >= pacLimit) then
                  pacLimit = oldPacLimit + tonumber(string.format("%.0f", (facIncConstCmd * pacMaxSetCmd / 1000)))
                  if (case2ZEF >= caseReset) then
                     ...
                     logCsvZEF(fileNameZEF,"1.2.0"..","..zeFac..","..zePac..","..pvPac..","..oldPacLimit..","..gridConn..","..pacLimit..",".."")
                  else
                     case2ZEF = case2ZEF + 1
                     logCsvZEF(fileNameZEF,"1.2.1"..","..zeFac..","..zePac..","..pvPac..","..oldPacLimit..","..gridConn..","..pacLimit..",".."")
                  end
               else
                  if (case2ZEF >= caseReset) then
                     case2ZEF = 0
                     ...
                     logCsvZEF(fileNameZEF,"1.2.2"..","..zeFac..","..zePac..","..pvPac..","..oldPacLimit..","..gridConn..","..pacLimit..",".."")
                  else
                     case2ZEF = case2ZEF + 1
                     logCsvZEF(fileNameZEF,"1.2.3"..","..zeFac..","..zePac..","..pvPac..","..oldPacLimit..","..gridConn..","..pacLimit..",".."")
                  end
               end
            ...
            elseif ((zeFac >= (zeFMinLoad - zeFLCriticalLoad)) and ((zeFac < (zeFMinLoad - zeFCriticalLoad)) and (maxPacLimit <= pacMaxSetCmd) and (zeIac < zeIacSetCmd))) then         ----less than changed to Greater than
               pacLimit = tonumber(minPacLimit + (facTuneStCmd * pacMaxSetCmd / 1000))
               pacLimit = tonumber(string.format("%.0f", pacLimit))
               if (pacLimit >= pacMaxSetCmd) then pacLimit = pacMaxSetCmd end
               if (case3ZEF >= caseReset) then
                  ...
                  logCsvZEF(fileNameZEF,"1.3.0"..","..zeFac..","..zePac..","..pvPac..","..oldPacLimit..","..gridConn..","..pacLimit..",".."")
               else
                  case3ZEF = case3ZEF + 1
                  logCsvZEF(fileNameZEF,"1.3.1"..","..zeFac..","..zePac..","..pvPac..","..oldPacLimit..","..gridConn..","..pacLimit..",".."")
               end
            ...
            elseif ((zeFac <= (zeFMinLoad + zeFUCriticalLoad)) and ((zeFac > (zeFMinLoad + zeFCriticalLoad)) and (maxPacLimit <= pacMaxSetCmd) and (zeIac < zeIacSetCmd))) then         ----less than changed to Greater than
               ...
               if (case4ZEF >= caseReset) then
                  ...
                  logCsvZEF(fileNameZEF,"1.3.2"..","..zeFac..","..zePac..","..pvPac..","..oldPacLimit..","..gridConn..","..pacLimit..",".."")
               else
                  case4ZEF = case4ZEF + 1
                  logCsvZEF(fileNameZEF,"1.3.3"..","..zeFac..","..zePac..","..pvPac..","..oldPacLimit..","..gridConn..","..pacLimit..",".."")
               end
            ...
            elseif ((zeIac >= zeIacSetCmd) and (maxPacLimit < pacMaxSetCmd)) then
               if (zeIac > zeIacMaxCmd) then
                  ...
                  if (case5ZEF >= caseReset) then
                     ...
                     logCsvZEF(fileNameZEF,"1.4.0"..","..zeFac..","..zePac..","..pvPac..","..oldPacLimit..","..gridConn..","..pacLimit..",".."")
                  else
                     case5ZEF = case5ZEF + 1
                     logCsvZEF(fileNameZEF,"1.4.1"..","..zeFac..","..zePac..","..pvPac..","..oldPacLimit..","..gridConn..","..pacLimit..",".."")
                  end
               end
            elseif ((zeFac <= (zeFMinLoad + zeFThreshold)) and (zeFac >= (zeFMinLoad - zeFThreshold)) and (maxPacLimit <= pacMaxSetCmd) and (zeIac < zeIacSetCmd)) then         ----less than changed to Greater than
               ...
               if (case6ZEF >= caseReset) then
                  ...
                  logCsvZEF(fileNameZEF,"1.4.2"..","..zeFac..","..zePac..","..pvPac..","..oldPacLimit..","..gridConn..","..pacLimit..",".."")
               else
                  case6ZEF = case6ZEF + 1
                  logCsvZEF(fileNameZEF,"1.4.3"..","..zeFac..","..zePac..","..pvPac..","..oldPacLimit..","..gridConn..","..pacLimit..",".."")
               end
            else
               case7ZEF = case7ZEF + 1
               logCsvZEF(fileNameZEF,"1.5.0"..","..zeFac..","..zePac..","..pvPac..","..oldPacLimit..","..gridConn..","..",".."")
            end
         end
      end
